...
- La sincronizacion con Riot se dispara desde la app: boton «Sincronizar todo» y un intervalo en el dashboard (`DashboardBackgroundSync`, cada 5 minutos por defecto, sobreescribible con `NEXT_PUBLIC_DASHBOARD_RIOT_SYNC_MINUTES`) solo mientras la pestana esta abierta; sin cron en repo. En modo automatico, el estado "Jugando" se revisa cada 5 minutos y rangos/partidas cada 10 minutos; en modo manual, "Jugando" se limita a una consulta cada 20 segundos y rangos/partidas a una consulta cada 1 minuto.
