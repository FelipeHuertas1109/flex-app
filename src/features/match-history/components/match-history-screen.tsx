...
import { MatchHistoryPanel } from "@/features/match-history/components/match-history-panel";
import { mapUserToShellUser } from "@/lib/auth/shell-user";
import { createClient } from "@/lib/supabase/server";

export async function MatchHistoryScreen() {
        ...
        <div className="flex h-[calc(100vh-200px)] items-center justify-center">
          <Panel className="max-w-md bg-black/40 p-8 text-center backdrop-blur-xl">
            ...
            <p className="mt-3 text-sm text-slate-300">
              Necesitas pertenecer a un grupo para consultar historial.{" "}
  ...
  return (
    <AppShell user={shellUser}>
      <MatchHistoryPanel snapshot={snapshot} />
    </AppShell>
  );
}
